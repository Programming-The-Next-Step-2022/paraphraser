...
* similar problem with formatting - when i kept output as numeric, rather than strings, it would give me errors saying it wasn't numeric. I tested the strings "1" and "0" rather than the numebrs 1 and 0, and commented out the error messages to non-numeric values in the function. This immediately fixed the problem again
...
* getting conditional statement to take more than one language input - had to make separate button that is the same for each language that can be made more or less formal. I didn't really like how this looked but I tried several ways of making some sort of OR statement but the function would give an error every time\

* remove split_sentences argument - did nothing and not useful when it worked
