...
test_that("translate returns a different string from input text", {
  ...
  expect_false(isTRUE(all.equal(translate_1$input_text, 
                                translate_1$output_text)))
})
...
test_that("translate returns correctly translated text", {
  ...
  expect_equal(translation_5$output_text, 
               "коричневая лиса перепрыгнула через черную кошку")
...
test_that("translate formality level is properly set", {
  ...
  expect_false(isTRUE(all.equal(translation_7$output_text, 
...
test_that("translate preserve_formatting correctly changes formatting", {
  ...
  expect_false(isTRUE(all.equal(translation_11$output_text, 
